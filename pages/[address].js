...
import ChaiFactory from "../artifacts/contracts/Soda.sol/SodaFactory.json";
import Chai from "../artifacts/contracts/Soda.sol/Soda.json";
...
export default function Detail({ Data, DonationsData }) {
  ...
  useEffect(() => {
    const Request = async () => {
      ...
      const contract = new ethers.Contract(Data.address, Soda.abi, provider);
  ...
  const DonateFunds = async () => {
    try {
      ...
      const contract = new ethers.Contract(Data.address,Soda.abi, signer);
        ...
        <div>
         ...
         Frizzling Soda Price
          ...
          <TextField
            ...
            type="number"
            margin="normal"
            placeholder="Frizzling soda price"
...
export async function getStaticPaths() {
  ...
  const getAllSoda = contract.filters.sodaCreated();
  const AllSoda = await contract.queryFilter(getAllSoda);

  return {
    paths: AllChais.map((e) => ({
      params: {
        address: e.args.sodaAddress.toString(),
      },
    })),
    fallback: "blocking",
  };
